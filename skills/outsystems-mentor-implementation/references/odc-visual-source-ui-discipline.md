...
**Read the layout the app renders, not the layout the docs default to.** The documented default above is what a *new empty screen* gets. It is not what the app in front of you is using, and it is not a safe premise for an existing app whose screens were generated together on some other layout. Before asking for any chrome that lives outside the content placeholder — a search bar, a top-bar control, a global command — determine which layout block actually renders. The rendered root carries the tell: `class="layout layout-side fixed-header"` is `LayoutSideMenu`, whatever the documented default says. The model-side read is `context_screens` (`odc-mentor-hardening.md` → `### TopMenu Navigation Prompts Must Reference The Menu Block, Not MainFlow Screens`, `**When to ask**`); the DOM read is the cheap confirmation once the app is deployed, and the two must agree before a chrome prompt is emitted. This is field-observed, not `Current official` — a way to read the app, not a platform contract. Skipping it produces the most expensive shape of failure this file guards: a search widget built into `LayoutTopMenu` on an app whose screens all render `LayoutSideMenu` is a valid element, publishes with zero errors, is instantiated by no screen, and reports success at every build-time gate (restaurant-app v2, 2026-08-30).
...
**Chrome is content of the container that renders, never a placeholder default.** A layout block's placeholder is a slot each consuming screen fills, so content the block authors into it is a default that every screen supplying its own replaces. Chrome parked there is built, passes TrueChange, survives publish, and is on no screen that matters — the mechanism is in `odc-mentor-hardening.md` → `### Block Chrome Must Be Real Block Content, Not Placeholder Default Content`. Emit the destination as a named container that renders its own content — for a fixed header, the navigation container inside the layout's header element — and emit it **unconditionally**. In particular never write a conditional destination such as *"use a placeholder if one exists"*: it hands Mentor an opt-out, the opt-out is the wrong branch, and nothing downstream can see it was taken, because the gate reads the emitted directive rather than the built result. Name the container, not a condition (restaurant-app v2, 2026-08-30 — one round lost to exactly that sentence).
...
- an expanding control inside a fixed bar (`field-tested hardening guidance`): a search suggestion list, filter menu, or notification panel authored as a child of a slim fixed header is lost at the bar's edge, because the bar is sized for one row and does not grow to fit it — and a short parent does not clip a child on its own, so the panel has to leave the flow rather than the bar get taller. Anchor it to a `position:relative` parent and give it `position:absolute; top:100%` with a `z-index`, so it overlays the page below the bar. Recipe in `odc-mentor-hardening.md` → `### A Dropdown Inside A Slim Fixed Header Clips Unless It Escapes The Bar`.
