...
# The generated catalog is published, so its provenance must not name a path
# that exists only on the maintainer's machine — it is unresolvable for every
# reader. The public docs roots identify themselves; the local mirror is
# recorded by role instead.
LOCAL_MIRROR_LABEL = "<local mirror of the public ODC reference docs>"


def neutral_root(path):
    return LOCAL_MIRROR_LABEL if path.as_posix().startswith("knowledge/") else path.as_posix()
...
def build_catalog(source_root, o11_source_root=None, o11_catalog=None):
    ...
    return {
        ...
        "source_search_roots": [neutral_root(path) for path in DEFAULT_SEARCH_ROOTS],
        "summary": summary,
        "widgets": widgets,
    }, []
