...
`Target app inventory:`, `Excluded scope:`, and
...
`Excluded scope:` is the do-not-build channel and is always sent. Each entry is
an ID with its disposition — `<ID> (deferred)`, `<ID> (out-of-scope)`,
`<ID> (accepted-risk)` — drawn from the same closed vocabulary as the plan's
`## Requirement Dispositions` table. Compose it from the excluded candidates in
the run's `screen-inventory.json` when one is on disk
(`outsystems-screen-inventory` prints them with their disposition word under
*do NOT design or implement* in its `--brief` output), plus every requirement
ID the patched plan dispositions terminally. When neither source names one,
send the literal `none recorded` — a blank field cannot be told apart from a
question nobody asked.

The invoked skill must:

- Treat the patched plan as the implementation source.
- Put every ID on `Excluded scope:` in Section 8 (Out of scope) of the 10-section Mentor spec and build none of them, partially or speculatively (CRITICAL CONSTRAINT 4). The field is a do-not-build list, not a backlog: an ID appearing there is a decision already taken upstream, not work to schedule.
