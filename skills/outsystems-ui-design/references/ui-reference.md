...
**Step A — Pick the Layout, THEN delete the default before adding it.** Load [`layouts.md`](layouts.md) and pick `LayoutSideMenu`, `LayoutTopMenu`, or `LayoutBlank` based on the screen's navigation pattern. **Don't default to `LayoutBlank`** — that gives you nothing. ⚠️ **Before adding your chosen Layout, delete the default `LayoutTopMenu` the platform scaffolds into every new screen** — see [`layouts.md#before-you-add-any-layout--delete-the-default-one-the-screen-ships-with`](layouts.md#before-you-add-any-layout--delete-the-default-one-the-screen-ships-with) for the inspect-delete-add rule. Skipping the delete is the #1 cause of "two layouts at the screen root" regressions even when the agent correctly picked a single Layout.
...
### FULL PATH naming (placeholders, arguments, event args)

> Absorbed 2026-08-05 from `outsystems-frontend-skills` `ui-frameworks/outsystems-ui/widget-conventions.md`
> (naming facts only — the widget-JSON build recipes stay excluded per the design-only boundary contract).

Whenever a block's placeholder, argument, or event argument is named anywhere —
region descriptions, `content[]` element descriptors, acceptance checklists —
use the **full path** form `<SourceBlock>.<Name>`:

| Kind | Full path form | Examples |
|---|---|---|
| Placeholder | `<SourceBlock>.<Placeholder>` | `Tabs.Header`, `Card.Content`, `Carousel.CarouselItems` |
| Argument / parameter | `<SourceBlock>.<Parameter>` | `Card.UsePadding`, `Counter.IsVertical`, `Columns2.GutterSize` |
| Event argument | `<SourceBlock>.<Event>.<Arg>` | `Tabs.OnTabChange.ActiveTab`, `Pagination.OnNavigate.NewStartIndex`, `DatePicker.OnSelected.SelectedDateTime` |

This is the format modelAPI patches already use downstream — bare names
(`UsePadding`, `NewStartIndex`) are legacy serializer wording; a design that
names properties full-path hands OMI exactly the identifiers it needs. Two
reference facts that travel with the naming: every block instance carries
`Arguments: []` and `PlaceholdersContent: []` even when empty, and placeholder
names are **case-sensitive** (a misspelled placeholder silently drops its
content — the layout renders with an empty slot).

⚠️ **`outsystems_hints.block` is the one exception and stays a bare catalog
token** (`"Card"`, never `"Card.UsePadding"`) — the blueprint validator
hard-fails any qualifier in it. Full-path naming applies to the *property and
placeholder references around* the block hint, never to the hint itself.
Per-block argument/placeholder names: see [blocks-index.md](blocks-index.md).
...
| `Breadcrumbs` | Default `BreadcrumbsItem` crumbs — **including a trail seeded by the app template** (a `LayoutSideMenu`-templated app ships `Dashboard` / `List` / `Detail`) | `Breadcrumbs.Content`, or the layout's `Breadcrumbs` placeholder for a template-seeded trail |
...
**This table is not a closed allow-list.** Treat *any* stock block that ships placeholder children the same way — and that includes chrome blocks **inherited from the app template**, not just blocks the spec instantiates. `Breadcrumbs` is the common miss: a `LayoutSideMenu`-templated app arrives with a default `Dashboard › List › Detail` trail already sitting in the layout's `Breadcrumbs` placeholder, so appending the real crumbs leaves all six rendering. When in doubt, inspect the block's existing children and strip them before populating.
