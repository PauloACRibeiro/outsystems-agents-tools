...
> **Harvested from:** `outsystems-frontend-skills` (OutSystems official, BSD-3-Clause) `ui-frameworks/outsystems-ui/patterns/interaction.md` (harvested 2026-07-29).
...
For format conventions and reference entity values, see [ui-reference.md](../ui-reference.md).
...
| `Carousel.ItemsPerSlide` | Record `CarouselItems{DesktopItems, TabletItems, PhoneItems}` (Integer each) | Responsive items-per-slide per device class. **Defaults to 1-per-slide if omitted** — set this whenever the design shows multiple cards visible at once (gallery row, peek/overlap of neighboring cards). **NOT a plain integer** — the canonical ODC Carousel doc types it as the `CarouselItems` record. |
...
10. **Event parameter names use FULL PATH format too** — e.g. `Parameter: "DatePickerRange.OnSelected.SelectedStartDate"`, `Parameter: "Tabs.OnTabChange.ActiveTab"`. See [`../ui-reference.md`](../ui-reference.md).
