...
> **Harvested from:** `outsystems-frontend-skills` (OutSystems official, BSD-3-Clause) `ui-frameworks/outsystems-ui/patterns/navigation.md` (harvested 2026-07-29).
...
For format conventions (FULL PATH parameters, required `Arguments: []` and `PlaceholdersContent: []`), see [ui-reference.md](../ui-reference.md).
...
- **If the app template seeds a default trail** — a `LayoutSideMenu`-templated app ships `Dashboard` / `List` / `Detail` in its `Breadcrumbs` placeholder — delete those default `BreadcrumbsItem`s before adding the real crumbs, or both trails render. See [`ui-reference.md` § Default children to strip](../ui-reference.md#default-children-to-strip-acceptance-item-per-block).
...
- `Pagination` renders Prev/Next as `<button>` elements internally and disables them at boundaries automatically. The `Previous`/`Next` placeholders swap only the icon glyph — button chrome, disabled state, and boundary behavior stay internal (see "Styling the buttons" above).
